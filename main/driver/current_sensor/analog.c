...
#define ESM_ANALOG_AVG_FRAME_COUNT    4U
...
static volatile uint32_t s_conv_done_total = 0U;                      //连续ADC回调触发总次数
static volatile uint32_t s_conv_done_last_read = 0U;                  //上一次读取最新电流时看到的总次数
static uint32_t s_avg_raw_ia_sum = 0U;
static uint32_t s_avg_raw_ib_sum = 0U;
static uint32_t s_avg_frame_count = 0U;
static bool s_initialized = false;
/*=========================================================================
连续ADC转换完成事件回调函数（需要注册到硬件），FOC控制任务注册这个回调后，每当连续
ADC转换完成一次，FOC控制任务就会被通知一次，回调函数会从事件数据中解析出电流采样的
原始值，并更新全局变量s_latest_raw_ia和s_latest_raw_ib，以及标志位s_latest_sample_valid
===========================================================================*/
                                                   ...
                                                   void *user_data)
{
    ...
    uint16_t raw_by_phase[ESM_ANALOG_FAST_PHASE_COUNT] = {0};
    bool got_by_phase[ESM_ANALOG_FAST_PHASE_COUNT] = {false};
    size_t sample_count;
    ...
    if (got_by_phase[0] && got_by_phase[1]) {
        ...
        if (s_avg_frame_count >= ESM_ANALOG_AVG_FRAME_COUNT) {
            s_latest_raw_ia = (uint16_t)(s_avg_raw_ia_sum / ESM_ANALOG_AVG_FRAME_COUNT);
            s_latest_raw_ib = (uint16_t)(s_avg_raw_ib_sum / ESM_ANALOG_AVG_FRAME_COUNT);
...
esp_err_t esm_drv_analog_init(const esm_drv_current_cfg_t *drv_cfg)
{
    ...
    if (cfg->etm_trigger_phase >= ESM_ANALOG_FAST_PHASE_COUNT) {
        ESP_LOGE(TAG, "ETM trigger phase out of range: %u", (unsigned)cfg->etm_trigger_phase);
        return ESP_ERR_INVALID_ARG;
    }
    for (size_t i = 0; i < ESM_ANALOG_FAST_PHASE_COUNT; i++) {
        if (cfg->fast_phase_index[i] >= ESM_ANALOG_FAST_PHASE_COUNT) {
...

    {
        ...
        adc_continuous_config_t cont_cfg = {
            ...
            .sample_freq_hz = foc_cfg->control.pwm_hz,
...
/*这个函数用于从adc回调写入的数组中取最新的样本，第二个参数是adc转换isr触发计数值，用来跟踪转isr和换完成情况 */
                                            ...
                                            uint32_t *conv_done_total)
{
    ...
    if (conv_done_delta != NULL || conv_done_total != NULL) {
        uint32_t total = s_conv_done_total;
        if (conv_done_delta != NULL) {
            *conv_done_delta = total - s_conv_done_last_read;
        }
        if (conv_done_total != NULL) {
            *conv_done_total = total;
        }
        s_conv_done_last_read = total;
    }
    return ESP_OK;
}
