...
#define ESM_FOC_ENCODER_ALIGN_RETRY           50       //编码器校零时的最大重试次数
#define ESM_FOC_ENCODER_ALIGN_DELAY_MS        20       //编码器校零时每次重试的延迟时间，单位毫秒
#define ESM_FOC_BUS_V_SAMPLE_DIV              10U      //总线电压降频采样，减小10kHz环内ADC负担
#define ESM_FOC_TASK_CORE_ID                  1        //FOC任务固定到CPU1，避免长期占用CPU0导致IDLE0喂狗失败
...
#define ESM_FOC_DEBUG_BYPASS_ADC              1        //1:旁路ADC采样用于性能定位，0:恢复真实采样
...
static esp_err_t esm_task_foc_align_encoder_zero(const esm_bsp_encoder_ops_t *encoder_ops)//FOC控制前对编码器进行校零，读取当前角度作为零点，结果范围是0~2PI
{
    ...
    for (retry = 0; retry < ESM_FOC_ENCODER_ALIGN_RETRY; retry++) {
        ...
        vTaskDelay(pdMS_TO_TICKS(ESM_FOC_ENCODER_ALIGN_DELAY_MS));
    }

    ESP_LOGE(TAG, "encoder align failed after %d retries", ESM_FOC_ENCODER_ALIGN_RETRY);
    return ESP_FAIL;
}
...
static void esm_task_foc_ctrl_entry(void *arg)  // FOC控制任务入口函数，等待周期触发，读取传感器数据，运行FOC算法，并输出PWM命令
{
    ...
    uint32_t bus_v_sample_div = 0;
    float bus_v = 24.0f;          //没读到电压就用默认值24V
#endif
    ...
    while (1) {
        ...
        sample.bus_v = 24.0f;
#else
        if (current_ops != NULL && current_ops->read_phase_current != NULL   //采三相电流，如果读电流失败就用默认值0A
            && current_ops->read_phase_current(&phase_current) == BSP_OK) {
            sample.ia = phase_current.ia;
            sample.ib = phase_current.ib;
            sample.ic = phase_current.ic;
        }
        if (bus_v_sample_div == 0U) {
            if (current_ops != NULL && current_ops->read_bus_voltage != NULL    //读总线电压，如果读电压失败就用默认值24V
                && current_ops->read_bus_voltage(&bus_v) == BSP_OK) {
                sample.bus_v = bus_v;
            } else {
                sample.bus_v = 24.0f;
            }
        ...
        bus_v_sample_div++;
        if (bus_v_sample_div >= ESM_FOC_BUS_V_SAMPLE_DIV) {
            bus_v_sample_div = 0U;
        }
#endif
        ...
        if ((now_tick - dbg_last_tick) >= pdMS_TO_TICKS(ESM_FOC_DEBUG_LOG_PERIOD_MS)) {
...
esp_err_t esm_task_foc_ctrl_start(void)// FOC控制任务的启动函数，进行必要的初始化和资源准备，然后创建任务
{
    ...
    if (current_ops == NULL || current_ops->calibrate_zero == NULL) {
        return ESP_ERR_INVALID_STATE;
    }
    ...
    if (current_ops->calibrate_zero() != BSP_OK) {
        return ESP_FAIL;
    }
    if (esm_task_foc_align_encoder_zero(encoder_ops) != ESP_OK) {
        return ESP_FAIL;
    }
